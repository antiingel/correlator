...
forecasts_file_path = os.path.join(os.pardir, os.pardir, "forecasts", "single_methods", "arima2_" + timestamp)
...
def train_model_arima(data, filler_cols=(1, 0)):
    ...
    for freq, (train) in data.items():
        ...
        for i, (ix, item) in enumerate(train.iterrows()):
            ...
            fit = forecast.auto_arima(ro.globalenv["train"], max_order=8, stepwise=False, approximation=False)
